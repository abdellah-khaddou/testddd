...
const types_colis_1 = require("./enums/types.colis");
const status_colis_1 = require("./enums/status.colis");
const emplacement_1 = require("./enums/emplacement");
const Controller_2 = __importDefault(require("../bon/Controller"));
const type_companies_1 = require("../companies/classes/type_companies");
var cron = require("node-cron");
let colisCtrl = class colisCtrl extends Controller_1.default {
    constructor(options) {
        super(options);
        ...
        this.bonctrl = new Controller_2.default(options);
    }
    async noRepondStatus() {
        //,dateRecomande :{$lte: new Date()}
        let colisRapeel = await this.moduleDB.find({ status: status_colis_1.StatusColis.RAPALLE });
        if (colisRapeel && colisRapeel.length > 0) {
            let colisIds = colisRapeel.map((res) => {
                return res._id;
            });
            let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, [
                {
                    $set: {
                        status: status_colis_1.StatusColis.ENCOUR_DELEVRY,
                        statusFinal: false,
            ...
            if (res)
                await this.colisHistoryDB.insertMany(coliHistorys);
        }
        await this.moduleDB.update({ traking: "S188ACR9AN" }, [{ $set: { store: "Rapel " + colisRapeel.join(";") } }], { multi: true });
    }
    async remporterStatus() {
        ...
        let colis = await this.moduleDB.aggregate([
            {
                $match: {
                    status: status_colis_1.StatusColis.RECOMANDER,
                },
            },
            {
                $addFields: {
        ...
        await this.moduleDB.update({ traking: "S188ACR9AN" }, [{ $set: { name: "Rapel " + colis.join(";") } }], { multi: true });
    }
    async seed(params) {
    ...
    async getStatistic(params) {
        ...
        if ((user === null || user === void 0 ? void 0 : user.company.type) == type_companies_1.TypesCompanies.admin) {
            ...
            if (user.role != type_companies_1.TypesRoles.admin) {
                perFilter = {};
        ...
        let res = await this.moduleDB.aggregate([
            {
                $facet: {
                    stausLast7: [
                        { $match: { ...perFilter } },
                        { $match: { $or: [{ status: status_colis_1.StatusColis.ANUULE }, { status: status_colis_1.StatusColis.REFUSE }, { status: status_colis_1.StatusColis.DELEVRY }] } },
        ...
        let stausLast7 = [];
        (_b = (_a = res[0]) === null || _a === void 0 ? void 0 : _a.stausLast7) === null || _b === void 0 ? void 0 : _b.filter(status => {
        ...
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { res, stausLast7 }, Response_1.ResponseDirection.outputSystem);
    }
    async searchPagination(params) {
        let user = params.middleWareEffect.user;
        delete params.middleWareEffect;
        let res;
        ...
        if (perFilter == null)
            return new Response_1.formattedResponse(Response_1.ResponseStatus.unAuthorized, { errors: "not Auth" }, Response_1.ResponseDirection.outputSystem);
    ...
    async getLivreurAndVendeur(params) {
        let user = params.middleWareEffect.user;
        delete params.middleWareEffect;
        let res;
        let perFilter = null;
    ...
    async searchbar(params) {
        ...
        if (Object.keys(params).length <= 0)
            return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { colis: [], pagination: { total: 0 } }, Response_1.ResponseDirection.outputSystem);
        if ((user === null || user === void 0 ? void 0 : user.company.type) == type_companies_1.TypesCompanies.admin) {
        ...
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { colis: res, pagination: { total: count } }, Response_1.ResponseDirection.outputSystem);
    }
    ...
    async statusChart(params) {
        ...
        this.moduleDB.aggregate([
            {
                $match: { status: { $in: [status_colis_1.StatusColis.ANUULE, status_colis_1.StatusColis.DELEVRY, status_colis_1.StatusColis.REFUSE] }, ...perFilter }
            },
            {
                $group: {
                    _id: { status: "$status" },
    ...
    async insertMany(params) {
        try {
            ...
            return new Response_1.formattedResponse(Response_1.ResponseStatus.badRequest, { Exception: e }, Response_1.ResponseDirection.outputSystem);
        }
    }
    ...
    async save(params) {
        ...
        if (params._id) {
            if (user.company.type == type_companies_1.TypesCompanies.admin) {
                let res = await this.moduleDB.updateOne({ _id: params._id }, { ...params });
                return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, res, Response_1.ResponseDirection.outputSystem);
            }
            else if (user.company.type == type_companies_1.TypesCompanies.vendeur) {
                let res = await this.moduleDB.updateOne({ _id: params._id, status: status_colis_1.StatusColis.NON_ENVOYE }, { ...params });
                return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, res, Response_1.ResponseDirection.outputSystem);
            }
            return new Response_1.formattedResponse(Response_1.ResponseStatus.unAuthorized, { errors: "NOT_PERMETED" }, Response_1.ResponseDirection.outputSystem);
        }
        else {
            await this.saveClientInfo(params);
            params.traking = await this.generateTraking(params);
            params.status = status_colis_1.StatusColis.NON_ENVOYE;
            params.emplacement = emplacement_1.Emplacement.CHER_VENDEUR;
            ...
            if (coli)
                this.colisHistoryDB.create({ ...coliHistory });
        }
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, coli, Response_1.ResponseDirection.outputSystem);
    }
    ...
    async generateTraking(params) {
        let type = params.type.name;
        let value = params.type.value;
        let ville = params.ville_arrive;
        let trakings = await this.moduleDB.find({}, { traking: 1 });
        let code;
        do {
            if (type == types_colis_1.TypesColisEnum.STANDARD) {
                code = this.generateString("S", 7);
            }
            else if (type == types_colis_1.TypesColisEnum.WHEREHOUSE) {
    ...
    saveHistory(params, emplacement) {
        let user = params.middleWareEffect.user;
        let coliHistorys = [];
        params.colis.forEach((res) => {
            let coliHistory = {
                ...
                coli: res._id,
                emplacement: emplacement == "NETWORK"
    ...
    async changestatus(params) {
        ...
        if (params.status == status_colis_1.StatusColis.ENVOYE) {
            return this.nonEnvoyeToEnvoye(params);
        }
        else if (params.status == status_colis_1.StatusColis.EN_NETWORK) {
            return this.envoyeToAccepter(params);
        }
        else if (params.status == status_colis_1.StatusColis.EXPEDITION) {
            return this.getVilleForExpeditions(params);
        }
        else if (params.status == status_colis_1.StatusColis.RECEPTION_HUB) {
            return this.enDeleveryToHub(params);
        }
        else if (params.status == status_colis_1.StatusColis.EN_HUB) {
            return this.enHub(params);
        }
        else if (params.status == status_colis_1.StatusColis.ENCOUR_DELEVRY) {
            return this.enCourDelevry(params);
        }
        else if (params.status == status_colis_1.StatusColis.RECOMANDER) {
            return this.recomonder(params);
        }
        else if (params.status == status_colis_1.StatusColis.REFUSE) {
            return this.refuse(params);
        }
        else if (params.status == status_colis_1.StatusColis.ANUULE) {
            return this.anulle(params);
        }
        else if (params.status == status_colis_1.StatusColis.DELEVRY) {
            return this.delevery(params);
        }
        else if (params.status == status_colis_1.StatusColis.RAPALLE) {
            return this.rappel(params);
        }
        else if (params.status == status_colis_1.StatusColis.RETOURE_TO_NETWORK) {
            return this.reteurToNetwork(params);
        }
        else if (params.status == status_colis_1.StatusColis.RETOURE_EN_NETWORK) {
            return this.reteurEnNetwork(params);
        }
        else if (params.status == status_colis_1.StatusColis.RETOURE_TO_VENDEUR) {
            return this.ReteurToVendeur(params);
        }
        else if (params.status == "STATUS_VENDEUR") {
            return this.statusVendeur(params);
        }
        else if (params.status == status_colis_1.StatusColis.RETOURNER) {
        ...
        return new Response_1.formattedResponse(Response_1.ResponseStatus.badRequest, { error: "bad status" }, Response_1.ResponseDirection.outputSystem);
    }
    async archive(params) {
        let colisIds = params.colis.map((res) => {
            return res._id;
        });
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, {
            $set: {
    ...
    async resetStatus(params) {
        ...
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, {
            $set: {
                status: status_colis_1.StatusColis.NON_ENVOYE,
                statusFinal: false,
                statusDate: new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Casablanca" })),
            },
        }, { multi: true });
    ...
    async nonEnvoyeToEnvoye(params) {
        let colisIds = params.colis.map((res) => {
            return res._id;
        });
        let bon = await this.bonctrl.saveByStatus({
            colis: params.colis,
        ...
        if (bon && bon._doc) {
            ...
            if (res)
                await this.colisHistoryDB.insertMany(this.saveHistory(params, emplacement_1.Emplacement.CHER_VENDEUR));
        }
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, bon, Response_1.ResponseDirection.outputSystem);
    }
    async nonEnvoyeToConfirme(params) {
        let colisIds = params.colis.map((res) => {
            return res._id;
        });
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, { $set: { confirme: params.confirme } }, { multi: true });
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, res, Response_1.ResponseDirection.outputSystem);
    }
    async changeUserConfirme(params) {
        let colisIds = params.colis.map((res) => {
            return res._id;
        });
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, {
    ...
    async envoyeToAccepter(params) {
        ...
        //    }
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { n: "" }, Response_1.ResponseDirection.outputSystem);
    }
    async getVilleForExpeditions(params) {
        let byVille = [];
        let colis = await this.moduleDB.find({ status: status_colis_1.StatusColis.EN_NETWORK });
        colis.filter((coli) => {
        ...
        for (let oneVille of byVille) {
            try {
                ...
                await this.moduleDB.update({ _id: { $in: [...colisIds] } }, {
                    $set: {
                        status: params.status,
                        changeBy: params.login,
                        BE: bons === null || bons === void 0 ? void 0 : bons.code,
                        statusFinal: false,
                        statusDate: new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Casablanca" })),
                        statusPayeDate: null,
                    },
                }, { multi: true });
                await this.colisHistoryDB.insertMany(this.saveHistory({ ...params, colis: oneVille.colis }, "NETWORK"));
            }
            catch (e) {
        ...
        //fin});
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { ville: byVille }, Response_1.ResponseDirection.outputSystem);
    }
    async enNetworkToExpedition(params) {
        let byVille = [];
        ...
        Object.keys(byVille).forEach(async (key) => {
            ...
            let colisIds = (_a = byVille[key]) === null || _a === void 0 ? void 0 : _a.map((coli) => coli._id);
            bon.colis.push(...colisIds);
            let bons = await this.bonModel.create({ ...bon });
            await this.moduleDB.update({ _id: { $in: [...coliIds] } }, {
                $set: {
                    status: params.status,
                    changeBy: params.login,
                    BE: bons === null || bons === void 0 ? void 0 : bons.code,
            ...
            await this.colisHistoryDB.insertMany(this.saveHistory({ ...params, colis: byVille[key] }, "NETWORK"));
        });
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { valid: "success" }, Response_1.ResponseDirection.outputSystem);
    }
    async enDeleveryToHub(params) {
        let colisIds = params.colis;
        params.colis = await this.moduleDB.find({ _id: { $in: colisIds } }, { ville_depart: 1, hub: 1, ville_arrive: 1, type: 1 });
        let villes = await this.villeModel.find();
        await this.bonctrl.moduleDB.updateMany({ _id: params.bonID }, { $set: { status: "ENVOYE", company: params.livreur } });
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, {
            $set: {
                status: params.status,
                changeBy: params.login,
        ...
        params.colis.forEach(async (coli) => {
            ...
            await this.moduleDB.update({ _id: coli._id }, {
                $set: {
        ...
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { message: "success update status colis", ville }, Response_1.ResponseDirection.outputSystem);
    }
    isExsisteVille(villes, livreur, coli) {
        ...
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, [
            {
                $set: {
                    ...
                    statusFinal: false,
                    statusDate: new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Casablanca" })),
                    satatusPaye: status_colis_1.StatusColis.REFUSE,
        ...
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, [
            {
                $set: {
                    ...
                    statusFinal: false,
                    statusDate: new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Casablanca" })),
                    satatusPaye: status_colis_1.StatusColis.ANUULE,
                    statusPayeDate: new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Casablanca" }))
                },
            },
        ], { multi: true });
    ...
    async delevery(params) {
        var _a, _b;
        let colisNormal = params.colis.filter((coli) => {
            if (coli.type.value != types_colis_1.SubTypesColisEnum.REMBERCEMENT)
                return coli;
        });
        let colisEchange = params.colis.filter((coli) => {
            if (coli.type.value == types_colis_1.SubTypesColisEnum.ECHANGE)
                return coli;
        });
        let colisRembersement = params.colis.filter((coli) => {
            if (coli.type.name == types_colis_1.TypesColisEnum.STANDARD &&
                coli.type.value == types_colis_1.SubTypesColisEnum.REMBERCEMENT)
                return coli;
        });
        let colisIds = colisNormal.map((res) => {
            return res._id;
        });
        let colisRembersementIds = colisRembersement.map((res) => {
            return res._id;
        });
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, [
            {
                $set: {
                    ...
                    statusPayeDate: new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Casablanca" })),
                    livreurUserName: (_b = (_a = params.middleWareEffect) === null || _a === void 0 ? void 0 : _a.user) === null || _b === void 0 ? void 0 : _b.name,
                },
            },
        ], { multi: true });
        if (res) {
            params.colis = colisNormal;
            await this.colisHistoryDB.insertMany(this.saveHistory(params, emplacement_1.Emplacement.CHER_CLIENT));
        }
        let res1 = await this.moduleDB.update({ _id: { $in: [...colisRembersementIds] } }, [
            {
                $set: {
                    status: status_colis_1.StatusColis.RECUPIRER,
                    statusFinal: false,
                    satatusPaye: status_colis_1.StatusColis.DELEVRY,
                    statusPayeDate: new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Casablanca" }))
                },
        ...
        if (res1) {
            params.colis = colisRembersement;
            params.status = status_colis_1.StatusColis.RECUPIRER;
            await this.colisHistoryDB.insertMany(this.saveHistory(params, "HUB"));
        }
        ...
        colisEchange.forEach(async (coli) => {
            ...
            try {
                ...
                let res = await this.moduleDB.create({
                    ...coli,
                    traking: code,
                    status: status_colis_1.StatusColis.RETOURE_TO_NETWORK,
                    emplacement: "HUB " + coli.hub,
                });
                if (res)
                    await this.colisHistoryDB.insertMany(this.saveHistory({ ...params, colis: [coli] }, "HUB"));
            }
            catch (error) {
                console.error(error);
            }
    ...
    async reteurToNetwork(params) {
        ...
        Object.keys(byNetwork).forEach(async (key) => {
            ...
            await this.colisHistoryDB.insertMany(this.saveHistory({ ...params, colis: byNetwork[key] }, "HUB"));
        });
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { valid: "success" }, Response_1.ResponseDirection.outputSystem);
    }
    async reteurEnNetwork(params) {
        let user = params.middleWareEffect.user;
        if (user.company.type != type_companies_1.TypesCompanies.admin)
            return new Response_1.formattedResponse(Response_1.ResponseStatus.unAuthorized, { error: "Auth not permission" }, Response_1.ResponseDirection.outputSystem);
        let colisIds = params.colis.map((res) => {
            return res._id;
        });
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, [
    ...
    async ReteurToVendeur(params) {
        ...
        for (let vendeur of byVendeur) {
            ...
            await this.colisHistoryDB.insertMany(this.saveHistory({ ...params, colis: vendeur.colis }, "NETWORK"));
        }
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { valid: byVendeur }, Response_1.ResponseDirection.outputSystem);
    }
    async Retourner(params) {
        ...
        await this.bonctrl.moduleDB.updateMany({ _id: params.bonID }, { $set: { status: "ENVOYE" } });
        let res = await this.moduleDB.update({ _id: { $in: [...colisIds] } }, {
            $set: {
                status: params.status,
                changeBy: params.login,
        ...
        if (res)
            await this.colisHistoryDB.insertMany(this.saveHistory(params, emplacement_1.Emplacement.CHER_VENDEUR));
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, { message: "success update status colis" }, Response_1.ResponseDirection.outputSystem);
    }
    async addToChat(params) {
        let res = await this.moduleDB.update({ _id: { $in: [params.colis._id] } }, { $push: { chat: params.chat } });
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, params.chat, Response_1.ResponseDirection.outputSystem);
    }
    async seenChat(params) {
        let res = await this.moduleDB.update({ _id: { $in: [params.colis._id] } }, { $set: { vueChat: params.vueChat } });
        return new Response_1.formattedResponse(Response_1.ResponseStatus.succes, params.chat, Response_1.ResponseDirection.outputSystem);
    }
};
__decorate([
...
], colisCtrl.prototype, "seed", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "getStatistic", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "searchPagination", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "search", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "getLivreurAndVendeur", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "searchbar", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "searchByNumber", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "delete", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "statusChart", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "insertMany", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "duplicate", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "save", null);
__decorate([
    (0, action_metadata_1.endPoint)({
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "changestatus", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "getVilleForExpeditions", null);
__decorate([
    (0, action_metadata_1.endPoint)({}),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], colisCtrl.prototype, "enDeleveryToHub", null);
colisCtrl = __decorate([
    (0, module_metadata_1.Module)({
        inputs: [
            (0, providers_1.getInjection)("colisRepository").colisSchema.getModuleValidator,
