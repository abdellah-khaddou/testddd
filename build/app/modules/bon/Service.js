...
let bonService = class bonService {
    constructor(options) {
        this.mainResolver = options.appResolver;
        this.bonCtrl = options.bonCtrl;
        this.mainResolver.addListener('bon', async ({ response, action, module, params }) => {
            //handling action with function from the controllers
            if (this.bonCtrl[action] && typeof this.bonCtrl[action] === 'function')
                return response(await this.bonCtrl[action](params));
