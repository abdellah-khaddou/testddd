...
let cashService = class cashService {
    constructor(options) {
        ...
        this.cashCtrl = options.cashCtrl;
        this.mainResolver.addListener('cash', async ({ response, action, module, params }) => {
            //handling action with function from the controllers
            if (this.cashCtrl[action] && typeof this.cashCtrl[action] === 'function')
                return response(await this.cashCtrl[action](params));
